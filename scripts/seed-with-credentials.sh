...
# Скрипт для заполнения справочников с явными учетными данными
set -e

API_URL="https://pocketbase.processlabs.ru/"
ADMIN_EMAIL="r.i.galeev@icloud.com"
ADMIN_PASSWORD="G@leevR0m@n"
...
if [ -z "$TOKEN" ]; then
    ...
    echo "Попробуем второй email..."
    
    ADMIN_EMAIL="r.i.galeev@gmail.com"
    AUTH_RESPONSE=$(curl -s -X POST "${API_URL}api/admins/auth-with-password" \
        -H "Content-Type: application/json" \
        -d "{\"identity\":\"$ADMIN_EMAIL\",\"password\":\"$ADMIN_PASSWORD\"}")
    ...
    if [ -z "$TOKEN" ]; then
        echo "❌ Ошибка аутентификации с обоими email"
        exit 1
    fi
fi
...
create_record() {
    ...
    if echo "$RESPONSE" | grep -q '"id"'; then
        ...
        echo "   ⚠️  Пропущено: $name"
        echo "      Ответ: $RESPONSE" | head -c 100
        return 1
    fi
}
...
echo ""
echo "📊 Проверьте созданные записи в Admin UI: ${API_URL}_/"
echo ""
